...
load("./codes/network/parameters/network_signed181_exprSet-block.1.RData")
...
if (T){
    for (minModSize in c(10,15,20)) {
      for (dthresh in c(0.05,0.1,0.15)) {
        for(ds in c(3:4)) { 
          pam = FALSE
...
moduleCors = moduleCors[c("Diagnosis","Age","Sex","AB42.ELISA",
...
id = match(rownames(datExp),attr$ensembl_gene_id)
attr= attr[id,]
tableSup = cbind(data.frame(mods),attr,kMEtable)
