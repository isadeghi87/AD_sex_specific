...
if(T){
  ...
  pres = read.csv("../TMT_consensus/results/tables/network/preservation/rosmap_RNA.csv")
  ...
  matchmod = read.csv("../TMT_consensus/results/tables/network/preservation/rosmap_rna_overlap.csv")
  matchmod = matchmod[matchmod$Protein==idx,]  
  ...
  df$tmt_color = names(mods)[match(df$tmt_mod,mods)]
    ...
    p = ggplot(df, aes(x = diagnosis,
      ...
      labs(y = "beta", x = "",
           title = "M7 - cellular localization") +
      scale_x_discrete()+
