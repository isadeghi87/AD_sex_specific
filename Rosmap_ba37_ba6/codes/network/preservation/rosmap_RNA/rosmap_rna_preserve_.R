...
if(T){
  ...
  ## load discovery proteomics
    ...
    p = ggplot(plotData,aes(x = size, y = z, 
      ...
      labs(y = "Z summary ",
           x = "module size",
           title = "Module preservation in transcriptome")+
      theme_bw()+
    ...
    p
    ggsave("./results/figures/network/rosmap_RNA_preserved.pdf",
           plot = p,
           width = 6,height =  4)
    
  ### save 
  write.csv(x = plotData,"./results/tables/network/preservation/rosmap_RNA.csv")
}
