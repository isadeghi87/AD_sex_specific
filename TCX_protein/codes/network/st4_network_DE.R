...
if(T){
  ...
  library(pacman)
  p_load(WGCNA,ggplot2,reshape2,dplyr,magrittr,ggthemes)
  

  setwd("C:/Users/crgcomu/Desktop/Iman/Brain_meta/analysis_rawCount/disease_specific/AD_PA_CTL/sex_specific/replication_proteomics_TCX/")
  sex = c("female","male")
  ...
  matched = matched[matched$dataset == "TCX",]
  res = data.frame()
  
  for (s in sex){
    celldat = subset(matched,sex ==s)
    id_match = unique(celldat$rep)
  ...
  p = ggplot(res, aes(x = module,
    ...
    labs(y = "beta", x = "", title = "TCX modules expression") +
    scale_x_discrete()+
