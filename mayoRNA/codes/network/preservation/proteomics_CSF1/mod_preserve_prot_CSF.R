...
if(T){
  ...
  for (s in sex){
    ...
    ## ploting
    pdf(paste0("./results/figures/network/preservation/csf1_preservation_",s,".pdf"),
        width = 6, height = 4)
    plot(x = plotData[,"z"],
         y = plotData[,"size"],
         col = 1, 
    ...
    ### save 
    plotData$sex = s
    plotData$dataset = "CSF1"
    res = rbind(res,plotData)
  }
  saveRDS(res,file = paste("./results/tables/network/preservation/csf1_preserve",s,".rds"))
