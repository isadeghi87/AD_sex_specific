...
if(T){
  ...
  for (s in sex){
    ...
    load(paste0("./replication_proteomics_CSF1/codes/network/parameters/finalNetwork_",s,".RData"))
    load("./replication_proteomics_CSF1/codes/network/parameters/Combined_for_network.Rdata")
    ...
    saveRDS(res.list,file= paste0("./results/tables/network/preservation/mayoRNA_csf1_overlap_",s,".rds"))
    ...
    h1 = ComplexHeatmap::Heatmap(res.or,
                                 ...
                                 row_title = paste0("CSF1 proteomics- ",s),
                                 row_title_side = "left",
                                 # width = unit(20,"cm"),
                                 # height = unit(7,"cm"),
    ...
    pdf(paste0("./results/figures/network/preservation/mayoRNA_csf1_overlap_",s,".pdf"),
        width = 8, height = 3)
    draw(h1)
    dev.off()
    ...
    df$sex = s
    df$dataset = "CSF1-proteomics"
    enrich = rbind(enrich,df)
    
    ## finding matched modules
    r = matchModules(gn1 = attr.disc$gene_name,mod1 = mods.discovery,
                     gn2= gene,mod2= mod.prot,omit = "grey")
  ...
  write.csv(match.res,"./results/tables/network/preservation/matched_modules_csf1.csv",row.names = F)
  saveRDS(enrich,"./results/tables/network/preservation/csf1_overlap.rds")
}
