...
if(T){
  ...
  for (s in sex){
    ...
    ## ploting
    pdf(paste0("./results/figures/network/preservation/mayoProteomics_preservation_",s,".pdf"),
        width = 6, height = 4)
    plot(x = plotData[,"z"],
         y = plotData[,"size"],
         col = 1, 
         bg = plotData$module,
         pch = 21,
         main = paste0("Temporal Proteomics- ",s),
         cex = 2.4,
    ...
    ### save 
    plotData$sex = s
    plotData$dataset = "TCX"
    res = rbind(res,plotData)
  }
  saveRDS(res,file = paste("./results/tables/network/preservation/tcx_preserve",s,".rds"))
}
